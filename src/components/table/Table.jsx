...
const Table = () => {
  ...
  const handleAddExpense = () => {
    if (!showDynamicRow) {
      ...
      // Initialize the dynamic row with default values
      setNewRow({
        date: new Date().toLocaleDateString(),
        type: "",
        category: "",
        ...
        <tbody>
          {tableData.map((row, index) => (
            <tr key={index} className="">
              ...
              <td className={`border border-[#5E5E5E] py-2 px-2.5 font-Roboto text-[11px] text-light tracking-[0.25px] font-normal text-center min-w-[75px] ${row?.reimbursable === "No" ? "bg-[#b91b1b33]": ""}`}>
                {row.reimbursable}
              </td>
              <td className="border border-[#5E5E5E] py-2 px-2.5 font-Roboto text-[11px] text-light tracking-[0.25px] font-normal text-center min-w-[100px]">
                {row.listing}
